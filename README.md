...
```
```sql
SELECT DATEADD(DAY,1, '2025-12-17') AS FutureDate
```
```sql
SELECT DATEDIFF(MONTH, '2024-06-01', '2025-02-10') AS MonthsDifference
```
```sql
...
```sql
SELECT PLAYERID,
COALESCE(MAX(CASE WHEN ROLE = 'ALL-ROUNDER' THEN ROLE END), '') AS ROLE_PLAYER,
COALESCE(MAX(CASE WHEN ROLE = 'BOWLER' THEN ROLE END), '') AS BOWLER
FROM CricketPlayers
...
## USING PIVOT
```sql
SELECT PLAYERID,[ALL-ROUNDER] AS ROLE_PLAYER,[BOWLER] AS BOWLER
FROM (
      SELECT PLAYERID, ROLE
	  FROM CricketPlayers
	  WHERE PLAYERID IS NOT NULL
	  ) AS SOURCE
PIVOT(MAX(ROLE)FOR ROLE IN ([ALL-ROUNDER], [BOWLER])) AS PIVOT_RESULT
```
